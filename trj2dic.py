...
def get_XYT_inDic(matfiles,start_frame_idx, isClustered, clustered_result, trunclen, isVisualize,isVideo, dataPath ,isSave, savePath, useVirtualCenter=False):  # get the time dictionary, such as vctime
    ...
    while frame_idx < np.int32(matfiles[-1][-7:-4])*trunclen:
        # pdb.set_trace()
...
def prepare_data_to_vis(isAfterWarpping,isLeft,isVideo, dataSource):
    if isAfterWarpping:
        ...
        if dataSource == 'Johnson':
            ...
            #subSampRate = 6
            # for linux
            matfiles         = sorted(glob.glob(outputPath+'filtered/' +'*.mat'))
            dataPath         = videoPath
            clustered_result = outputPath+'ssc/Complete_result'
            savePath         = outputPath+'ssc/'
...
# def trj2dic_main(isVideo, dataSource):
    ...
    start_frame_idx = (np.int32(matfiles[result_file_Ind*25][-7:-4])-1)*trunclen #start frame_idx

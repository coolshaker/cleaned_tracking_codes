...
#     dataSource = 'DoT'
    ...
    if len(previousLastFiles)>0:
        frame_idx = len(previousLastFiles)*trunclen
    ...
    #frame_idx=4000 # starting frame 

    # -- set low number of frames for testing
    # nframe = 1801

    """Begin the loop"""
    #if isVideo: cap.set ( cv2.CAP_PROP_POS_FRAMES , max(0,frame_idx))

    while (frame_idx < nframe/subSampRate): #frame_idx: frame index for converted video (5 fps for Fulton, 6 fps for Johnson)
    #while (frame_idx < 100):       
        ...
        if isVideo:
            try:
                cap.set ( cv2.cv.CV_CAP_PROP_POS_FRAMES , max(0,frame_idx*subSampRate))
                status, frame[:,:,:] = cap.read() # read one frame every subSampRate frames
                
            except:
                print "exception!!"
                frame_idx = nframe/subSampRate
                continue
        ...
        if frame_idx % detect_interval == 0: 
            ...
            corners = cv2.goodFeaturesToTrack(frameL,mask=mask,**feature_params)  #get the corners
            # if frame_idx==69605:
            #     pdb.set_trace()
        ...
        if  ((frame_idx>0) & ((frame_idx) % trunclen == 0)) or (frame_idx==nframe/subSampRate): 
            print "saving===!!!"+str(frame_idx/trunclen)   

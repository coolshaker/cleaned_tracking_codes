...
def trj_filter(x, y, t, xspeed, yspeed, blob_index, mask, Numsample , minspdth = 15, fps=5):
    ...
    for i in range(Numsample):
        #print sum(x[i,:])
        if sum(x[i,:]!=0)>3*fps:  # new jay st  # chk if trj is long enough
...
def prepare_input_data(dataSource):
    ...
    if dataSource == 'Johnson':
        ...
        matfilepath = outputPath
        savePath    = outputPath+'filtered/'       # matfilepath = '../tempFigs/roi2/'
